...
describe("readFrames — 視覚センサー（文字起こししない・生の base64）", () => {
  it("source 未設定/空白なら空（視覚オフ）", () => {
    expect(readFrames()).toEqual([]);
    expect(readFrames("")).toEqual([]);
    expect(readFrames("   ")).toEqual([]);
  });

  it("存在しないパスなら空（壊さず黙る）", () => {
    expect(readFrames("/no/such/file.png")).toEqual([]);
  });

  it("ファイルパスなら中身を base64 で1枚返す", () => {
    const dir = mkdtempSync(join(tmpdir(), "frame-"));
    const f = join(dir, "a.png");
    const bytes = Buffer.from([1, 2, 3, 4]);
    writeFileSync(f, bytes);
    try {
      expect(readFrames(f)).toEqual([bytes.toString("base64")]);
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });

  it("ディレクトリなら最新更新の画像1枚（非画像は無視）", () => {
    const dir = mkdtempSync(join(tmpdir(), "frame-"));
    ...
    try {
      expect(readFrames(dir)).toEqual([newerBytes.toString("base64")]);
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
