...
import { runPlanProcessor } from "../src/roles/plan-processor.js";
...
describe("plan processor（前判定・マイルストーン照合）", () => {
  ...
  it("満たされたマイルストーンを順に✓し current を未完の先頭へ進める", async () => {
    ...
    const result = await runPlanProcessor(llm, {
      plan: plan(),
      worksBody: "段落1の本文。段落2の本文。",
    });
    expect(result.completedIds).toEqual(["m1", "m2"]);
    expect(result.plan.milestones.map((m) => m.done)).toEqual([true, true, false]);
    expect(result.plan.current).toBe("m3");
  ...
  it("全マイルストーン達成で allDone・current=null・達成ログを足す", async () => {
    ...
    expect(result.plan.current).toBeNull();
    expect(result.plan.milestones.every((m) => m.done)).toBe(true);
    expect(result.plan.log.some((e) => e.text.includes("達成"))).toBe(true);
  });
  ...
  it("満たされたものが無ければ計画を変えない", async () => {
    ...
    const result = await runPlanProcessor(llm, { plan: input, worksBody: "" });
    expect(result.completedIds).toEqual([]);
    expect(result.allDone).toBe(false);
    expect(result.plan).toBe(input); // 同一参照＝無変更
  ...
  it("判定のパース失敗は未達として扱い✓しない", async () => {
    ...
    const result = await runPlanProcessor(llm, { plan: plan(), worksBody: "なにか" });
    expect(result.completedIds).toEqual([]);
    expect(result.plan.milestones.every((m) => !m.done)).toBe(true);
  });
