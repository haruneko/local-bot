...
export const memoActor: ActorRunner = {
  ...
  criteria:
    ...
    "過去に同じことを記録していないか確認したい・集中で取り組み中の成果物を書き進める、のとき。" +
    "**単発の事実・記録が領分。多段で取り組む「目標そのもの」は段取り（steps）の領分なので memo では扱わない**（段取りノート goals/ にも書かない）。" +
    "**転記できる実物（会話・このターンの調べ物の実結果）があるときだけ書く＝自分の知識では書かない**。" +
    "調べる依頼だけ（記録まで頼まれていない）では起動しない。調べて控える依頼なら webSearch と同時に起動してよい" +
    "（memo は調べたあとに動くので、その実結果を転記できる）。" +
    "雑談・相槌・感情では起動しない。",
  run: (llm, input) => {
