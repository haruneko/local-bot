...
export async function recallRecognizeTarget(
  ...
  // goals/ は steps 所有の派生ビュー＝memo の対象にしない（steps の領分・取り合い防止）。
  const notSteps = (p: string) => !p.startsWith("goals/");
  const vhits = (await memoIndex.recall(intent, topK)).filter((h) => notSteps(h.path));
  // hybrid: 「名前そのまま」クエリは意味ベクトルが弱いので字句一致（ファイル名）を RRF 融合する。
  // ただしゲート（強い字句一致のときだけ）で話題クエリへのノイズ注入を防ぐ（eval:retrieval で検証）。
  const all = (await memoIndex.list()).filter((e) => notSteps(e.path));
