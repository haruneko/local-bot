import type { Milestone, StepsState } from "./state.js";
import { stepsSlug } from "./state.js";
...
export type StepsOp = {
  op:
    ...
    | "log"
    | "noop";
  /** 対象の計画 id（省略時 = focusSteps）。new_goal では無視（生成する） */
  stepsId?: string;
...
function nextMilestoneId(state: StepsState): string {
...
export function applyStepsOp(
  ...
  if (!state) return null; // 以降の op は既存 steps が前提
  ...
  switch (op.op) {
    ...
    case "activate": {
      // 開始/再開: focusSteps の付け替えは orchestrator が行う。ここでは current が空なら
    ...
    case "shelve": {
      // 棚上げ: focusSteps から外すのは orchestrator。steps 自体は active のまま残す。
