...
| 係 | 正体 | やること | いつ動く | 状態 |
|---|---|---|---|---|
| **手配り** dispatcher | LLM 判断（集中のみ） | current タスクを**どの手で進めるか**選ぶ（synthesize/webSearch/urlBrowse/memo）。どの手でもできなければ **none**。直近の行動を見て「調べた→次は書く」に移る | 集中の実行時 | 実装済み |
...
### dispatcher の賢さ：none と直近行動（2026-06-19）

dispatcher に2つ持たせて、空回りと手の無いものを断つ：

1. **none（手が無い）→ 入口で塞ぐ**: current マイルストーンを**どの手でもできない**（楽器の練習・実際の配線・買い物など物理/現実の行動）なら dispatcher は `none` を返す。orchestrator は none を受けたら**その段取りを集中から外す（shelve）**＝grind させない。idle が拾っても・明示 activate でも、最初の集中ターンで none → 即 shelve。＝作る側の手ゲート(new_goal)の**実行側バックストップ**（記憶から再生・明示依頼で入った un-doable もここで止める）。
2. **直近行動を見て「調べた→書く」**: dispatcher に**直近でやったこと**（recall/作業記憶）を見せる。もう必要な情報を**調べてあるなら、また webSearch せず synthesize で結論/選定を works に書く**。＝研究の「調べっぱなしで結論しない」無限ループを断つ（実機で観測：ESP32 のピン配置を延々検索し続け、選定を書かず milestone が未完のまま）。書く中身は**取れた実データ由来**（作話でない）。

⚠️ (2) は synthesize に**実データが文脈に載っている**ことが前提（載ってないと作話に戻る）＝研究結果が doer の入力に来るかが肝。
...
### 段取りは「自分の手でできること」だけ（2026-06-19）

**問題**: 作る人（new_goal）が dispatcher（手を選ぶ人）と違って**使える手を知らない**ため、楽器の練習・買い物・物理工作のような**手の無いゴール**を平気で作る。idle がそれを拾っても自律で完遂できず空回りする（実機で観測）。

**設計**: **作る人にも dispatcher と同じ「使える手」を教える**（STEPS_SYSTEM の new_goal 誘導）。エバの手は ①作る(synthesize) ②調べる(webSearch) ③URL を読む(urlBrowse) ④確かな事実を控える(memo) の4つだけ。段取りは**この手で進められることに分解**し、各マイルストーンも手のどれかで実行できる単位にする。**手の無いこと（練習・買い物・掃除・現実の行動・物理工作）は段取りにしない**（自分でできない＝必要なら memo に控えるだけ）。

**実機の効き（2026-06-19）**: 手を教えたら**できる系（書く・調べる）の分解が明確に良化**（手で実行できる単位に割れる）。ただし**明示で頼まれた un-doable は依然作る**（「練習」を「調べる/メモ」に言い換えて scope）。強く"断れ"と書くと正当な依頼まで過剰拒否するリスクがあるため、断らせるのでなく**手の範囲に scope して作る**を許容（純物理のマイルストーンは正直に停滞する＝嘘をつかない）。**実害＝自律 idle が"進められない段取り"を拾って空回り**は、別途「idle は前に進められる段取りだけ拾う」で断つ（残課題）。
