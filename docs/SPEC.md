...
1.5. **plan processor（前判定・`集中` かつ `focusPlan` のときのみ）** → 成果物(`works/<id>.md`)と計画を突き合わせ、current から前へ「この milestone は満たされたか」を狭い yes/no で判定（×N）→ **満たされた milestone を機械が✓・current 前進**（LLM は判定だけ・構造更新はコード）→ 全✓なら集中を締める（`focusPlan` クリア）。doer に渡す `currentTask`（current milestone 本文）を確定。`src/roles/plan-processor.ts`。＝計画の前進を「plan actor が発火するか」の博打でなく毎ターンの機械フェーズにする（DECISIONS §集中モード）
2. **actor pool** → 判断系（memo/webSearch/plan/synthesize）は **multi-label 1発**で起動判定（`runMultiLabelActivator`・各 criteria を同時判定）＋客観ゲート（urlBrowse=URL 正規表現）→ 起動した actor が実行 → `ctx.actions` に追加
...
6. **State 更新**（機械導出）: `user_message`→`対話`／heartbeat+`focusPlan`→`集中`／他→`静穏`。集中の入口（plan actor の `activate` で focusPlan 確定・`shelve`/`retire` で手放す）・強制ギプス・見限りもここで処理
7. **`TurnContext` 破棄**
...
| `webcam` | カメラ映像取得（未実装） |
| `plan` | 計画を扱う手（効果器）。backlog 一覧を見て `planId` で任意の計画を target し op を1つ（new_goal/activate/shelve/retire/complete/reopen/set_current/add_milestone/log）。`activate` で開始・再開（集中へ）、`shelve`/`retire` で手放す。日々の milestone 前進は plan processor が自動でやる。構造保証はコード（`data/plans/<id>.json`・markdown は派生ビュー）。設計は [PLAN-FACULTY.md](PLAN-FACULTY.md) |
| `synthesize` | 想起＋外部＋感性（内心/関心事）を統合して成果物（歌詞・読書メモ・まとめ・文章）を**生成**し `data/notes/works/<planId\|slug>.md` へ append 外化（生成が役割の唯一のレーン・memo は転記）。`集中` では `currentTask`（current milestone）1個だけを的に書く（計画全体は渡さない） |
...
**判断系（`criteria` を持つ actor＝memo/webSearch/plan/synthesize）= multi-label 1発**（`runMultiLabelActivator`）:
...
- 4基準を同時に見るので memo/plan/synthesize の三つ巴の過剰発火を抑える（別々判定は各 gate が他を知らず各自 active と言いがち）。実測で別々より正確かつ速い。
