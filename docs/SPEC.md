...
```json
{
  ...
  "recallDistance": { "fullMax": 0.55, "summarizeMax": 0.72, "vagueMax": 0.85 },
  ...
  "ollamaThink": false,
  "ollamaNumCtx": 30000,
  "languageNumPredict": 400,
  "timeZone": "Asia/Tokyo",
  "actors": {
    "recall":    { "enabled": true,  "channels": ["conversation", "inner_state"] },
    "remember":  { "enabled": true,  "channels": ["conversation", "inner_state"] },
    "forget":    { "enabled": true,  "channels": ["conversation", "inner_state"] },
    "memoWrite": { "enabled": true,  "channels": ["conversation", "inner_state"] },
    "memoRead":  { "enabled": true,  "channels": ["conversation", "inner_state"] },
    "webSearch": { "enabled": true,  "channels": ["conversation"] },
    "urlBrowse": { "enabled": true,  "channels": ["conversation"] },
    "webcam":    { "enabled": false, "channels": ["conversation", "inner_state", "image_feed"] }
  },
  "stateConfig": {
...
1. **プリプロセス** → 想起クエリ決定 → エピソード/意味記憶/memoIndex 想起 → `TurnContext` 生成（`stateConfig` でフィルタ量決定。元データ変更なし）
   - 想起クエリ: `lastUserContent → lastSpeech → innerState → null`。null のとき recall 系全スキップ
...
5. **内心更新**（内省と同条件）→ `updateInnerState` で `innerState` を書き換え（`prevInnerState` は1文に圧縮して渡す）→ `data/state.json` に永続化
