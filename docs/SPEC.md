...
### 6.2 カテゴリとサブエージェント（v0.5）

ジャッジが選ぶ `ACTION.kind`: `none` | `memory` | `research` | `express`

| kind | サブエージェント | 実行層 |
|------|------------------|--------|
| `memory` | 記憶サブエージェント | in-process（下表） |
| `research` | 探索サブエージェント | MCP（読取系ツール） |
| `express` | 発信サブエージェント | MCP（書込系ツール）+ 共有言語機能 |
...
入力は `RunActionInput { ctx, episodes, episodeRecallTopK, mcp, toolCatalog, expressDryRun }`。
