...
1.5. **steps processor（前判定・`集中` かつ `focusSteps` のときのみ）** → 成果物(`works/<id>.md`)と計画を突き合わせ、current から前へ「この milestone は満たされたか」を狭い yes/no で判定（×N）→ **満たされた milestone を機械が✓・current 前進**（LLM は判定だけ・構造更新はコード）→ 全✓なら集中を締める（`focusSteps` クリア）。doer に渡す `currentTask`（current milestone 本文）を確定。`src/roles/steps-processor.ts`。＝計画の前進を「steps actor が発火するか」の博打でなく毎ターンの機械フェーズにする（DECISIONS §集中モード）
2. **actor pool** → 判断系（memo/webSearch/steps/synthesize）は **multi-label 1発**で起動判定（`runMultiLabelActivator`・各 criteria を同時判定）＋客観ゲート（urlBrowse=URL 正規表現）→ 起動した actor が実行 → `ctx.actions` に追加
   - `集中` の doer（synthesize）には**計画全体でなく `currentTask`（current milestone 1個）だけ**を渡す＝先のステップへの先走りを防ぐ（doer steering）
...
| `webcam` | カメラ映像取得（未実装） |
| `steps` | 計画を扱う手（効果器）。backlog 一覧を見て `stepsId` で任意の計画を target し op を1つ（new_goal/activate/shelve/retire/complete/reopen/set_current/add_milestone/log）。`activate` で開始・再開（集中へ）、`shelve`/`retire` で手放す。日々の milestone 前進は steps processor が自動でやる。構造保証はコード（`data/steps/<id>.json`・markdown は派生ビュー）。設計は [STEPS-FACULTY.md](STEPS-FACULTY.md) |
