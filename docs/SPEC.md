...
- **State は言語野の宣言でなく観測事実から機械導出する**（`src/orchestrator/turn.ts`）: `user_message` → `対話`（集中は中断されここへ戻る）／ heartbeat で取り組み中の `focusSteps` があれば → `集中`／ それ以外 → `静穏`。言語野は State を出力しない（旧 `NEXT_STATE` 出力は廃止）
- 集中の制御: `focusStreak` が `MAX_FOCUS_STREAK`(=10) に達したら強制ギプス（focusSteps を手放し休む）／進捗が `MAX_FOCUS_STALL`(=6) 伸びなければ見限り（retired）。DECISIONS §集中モード
...
1. **プリプロセス** → 想起クエリ決定 → エピソード/意味記憶/memoIndex 想起 → `TurnContext` 生成（`stateConfig` でフィルタ量決定。元データ変更なし）
   - 想起クエリ: user_message は `lastUserContent ＋ concern`（concern を合成して研ぐ＝concern-aware・能動 recall の研いだクエリを機械代替）／ heartbeat は `lastUserContent → lastSpeech → concern → affect → null`。null のとき recall 系全スキップ
2. **actor pool**
   - **`対話`/`静穏`（反応モード）**: 判断系（memo/webSearch/steps/synthesize）は **multi-label 1発**で起動判定（`runMultiLabelActivator`）＋客観ゲート（urlBrowse=URL 正規表現）。`activate()` 入力は mini-context（直近 `ACTOR_CONTEXT_TURNS=3` ターン + 最新発話 + 内心）
   - **`集中`（実行モード）**: 汎用 activator の代わりに **dispatcher**（`src/roles/steps-dispatch.ts`）が `currentTask`（current milestone）に合う手を1つ選び（synthesize/webSearch/urlBrowse/memo）、その手だけ実行する＝手の取り違え（生成器に研究を作話させる等）を防ぐ。steps 管理 actor は集中では起動しない（管理は対話/静穏）。doer には**計画全体でなく `currentTask` だけ**渡す（先走り防止）。`currentTask` はターン頭で current milestone を読むだけ（判定はしない）
2.5. **受け入れ判定（実行のあと・`集中` かつ `focusSteps` のときのみ）** → 今回の行動の**実結果**（作った本文・調べたデータ）＋成果物(`works/<id>.md`)で「current milestone は実際にやれたか」を狭い yes/no で判定（`src/roles/steps-processor.ts`）→ **やれていれば機械が✓・current 前進・全✓なら完了畳み**（`focusSteps` クリア）。できなかった（失敗・空振り）なら進めない＝偽の done を作らない。**判定役は doer とは別人格**（やった本人に合否を出させない＝作話防止）。達成済みの段取りは再判定しない（完了畳み）
3. **language-agent** → 全 facts を受け取り発話生成 → `ctx.speech` を設定（State は出力しない）→ **口の効果器 `OutputChannel.say(speech, artifacts)` で即出力**（効果器モデル・§7.5。移行中は `TurnResult.speech` の pull も併存）
4. **内省くん**（`shouldPersistIntrospection(ctx)`）→ `introspection: string` → エピソード追記（発話 say の**後**＝post-hoc。`ctx.actions` を読むので記憶は正確）
...
6. **State 更新**（機械導出）: `user_message`→`対話`／heartbeat+`focusSteps`→`集中`／他→`静穏`。集中の入口（steps actor の `activate` で focusSteps 確定・`shelve`/`retire` で手放す）・強制ギプス・見限りもここで処理
7. **`TurnContext` 破棄**
...
| `webcam` | カメラ映像取得（未実装） |
| `steps` | 計画を扱う手（効果器）。**対話・静穏で**管理する（集中では起動しない＝執行に専念）。backlog 一覧を見て `stepsId` で任意の計画を target し op を1つ（view/new_goal/activate/shelve/retire/complete/reopen/set_current/add_milestone/log）。`view` で報告、`activate` で開始・再開（集中へ）、`shelve`/`retire` で手放す。new_goal は既存と字句照合して重複を作らない。日々の milestone 前進は受け入れ判定が自動でやる。構造保証はコード（`data/steps/<id>.json`・markdown は派生ビュー）。設計は [STEPS-FACULTY.md](STEPS-FACULTY.md) |
| `synthesize` | 想起＋外部＋感性（内心/関心事）を統合して成果物（歌詞・読書メモ・まとめ・文章）を**生成**し `data/notes/works/<stepsId\|slug>.md` へ append 外化（生成が役割の唯一のレーン・memo は転記）。`集中` では `currentTask`（current milestone）1個だけを的に書く（計画全体は渡さない） |
...
**判断系（`criteria` を持つ actor＝memo/webSearch/steps/synthesize）= multi-label 1発**（`runMultiLabelActivator`）:
- 各 actor の `criteria`（起動条件テキスト）を1つの system に並べ、共有文脈（criteria 系チャンネルの和）で **1回の LLM 呼び出し**で全 actor の `{active, intent}` を同時判定する。出力は actor 名をキーにした入れ子 JSON（`format` で grammar 強制・$ref ラッパは付けず inline）。
- 4基準を同時に見るので memo/steps/synthesize の三つ巴の過剰発火を抑える（別々判定は各 gate が他を知らず各自 active と言いがち）。実測で別々より正確かつ速い。
- パース失敗・リトライ失敗 → 該当ターンは空（起動なし）にフォールバック。

**客観ゲート（`activate` を持つ actor）= 機械判定**（DECISIONS「判定系は必ず LLM／ただし客観条件で決まる起動は機械ゲート可」）:
- `urlBrowse` は会話/計画の URL 有無で起動（正規表現・LLM 不要）。recall は常時（背景想起・preprocess）。
...
作用は所有 action が**自分の効果器**で起こす（受容器⟷効果器の対称・[CONCEPT](CONCEPT.md)／[DECISIONS](DECISIONS.md) §効果器）。発話＝「口」の効果器。
...
- 状態（[EFFECTORS-STEPS](EFFECTORS-STEPS.md)）: B1（口）＋B2（say も効果器・全 effect 所有確認）＋B3（pull を出力路から外し log 専用）＝**完了**。残る uniformity 改善＝手/steps の effector を module-fn から注入式へ（任意・特別経路ではないので非ブロッキング）。
...
| T-IS06 | buildRecallQuery: user_message は発話＋concern を合成（concern 空なら発話のみ） |
| T-FS01 | State 導出: user_message は focusSteps があっても `対話` に割り込む（集中の中断・focusSteps は保持） |
| T-FS02 | State 導出: heartbeat + focusSteps あり → `集中`、focusStreak 加算 |
| T-FS03 | State 導出: heartbeat + focusSteps なし → `静穏`、focusStreak リセット |
| T-FS04 | 強制ギプス: focusStreak が MAX_FOCUS_STREAK(=10) で focusSteps を手放し `静穏` |
