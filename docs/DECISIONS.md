...
~~`recall` 行動のヒット要約も LLM（`summarizeRecallActionHits`）。~~ **2026-06-18 廃止**: recall の多段ループと要約 LLM をやめ、ベクトル検索の上位を**機械的に top-2 提示**する（`runRecall`）。常時走る背景 recall と二重で費用倒れだったため（約3倍速・想起内容も決定的に。docs/research/recall-mechanical-2026-06-18.md）。下記「取り込み（主犯）」の機構もこれで消えた。
...
- **集中の3段制御（2026-06-13 改訂。旧「plan op 成功で集中に強制上書き」は廃止）**:
  ...
  - **見限り＝進捗ベース卒業（2026-06-16）**: 疲労（focusStreak＝休む・goal は残し後で戻る）とは別に、**進捗が出ない goal を見限る**。`state.json` の `focusStall`（進捗の出ない集中ターン数）が `MAX_FOCUS_STALL`(=6) に達したら、その plan を `retired:true` にして手放す（卒業ログを1行追記）。retired は集中の対象から外れ、疲労と違って**自動復帰しない**。進捗の測り方は純関数 `planProgress`（完了マイルストーン数＋ログ数）＝伸びれば停滞リセット・`focusBaseline` に最高進捗を記憶。判定は純関数 `evaluateFocusGraduation`（`src/plan/focus.ts`）、適用は `turn.ts applyFocusGraduation`。`MAX_FOCUS_STALL < MAX_FOCUS_STREAK` ＝進捗の出ない goal は休む前に見限られる。「達成不能な集中への張り付き」の解。
...
### plan processor（前判定・計画実行の背骨）（2026-06-19）

**実機計測で判明した故障**: 集中で heartbeat を連打しても**計画が前進しない**。プローブ（マイルストーン付き計画を集中で8連打）で、生成(synthesize)は毎ターン works に書くのに **done=0/4 のまま固定→空回り→見限り(retired)**。原因の連鎖：①マイルストーンを進める唯一の経路 `plan` actor が **multi-label で「synthesize が回ってるから plan 不要」と判定され8回中1回しか発火しない**。②その1回も `complete` でなく `log` を選ぶ。③ current が動かず計画チャンネルが永遠に「m1」を指す→ synthesize が同じ所を再記述しループ。④進捗センサ(`planProgress`=done+log)は works を見ないので、書いてるのに「進捗ゼロ」と盲目見限り。＝**前進を「plan actor が発火するか＋正しい op を選ぶか」という広い LLM 判断に賭けていたのが誤り**（codex 教訓：進捗追跡と step 遷移はモデルに委ねずシステムが握る）。

**解＝plan processor を前判定の機械フェーズに**（`src/roles/plan-processor.ts`・SPEC §4.2 phase 1.5）。集中の毎ターン頭で、成果物(works)と計画を突き合わせ、current から前へ「この milestone は成果物の中で達成されたか」を**狭い yes/no で1つずつ**判定（分割統治＝小モデルで固い）。満たされた milestone を**機械が✓・current 前進**、全✓なら締める。LLM の仕事は二値判定だけ・構造更新（✓・current・達成ログ）はコード。これで「plan actor 発火の博打」も「盲目見限り」も消える。plan actor は計画の**作成/編集**役に縮小（前進は processor 担当）。配置は**前判定**を選択：完了が「決めてから締める」きれいな流れになり、書く人が常に正しい milestone を書く保証が最強（前ターンまでの全 works から毎ターン頭で実態に合わせる＝ズレ窓ゼロ）。「判定の配置(前/後)」と「喋りの任意化」は別レバー＝混同しない。

**doer steering（#1）**: 集中の doer(synthesize)には**計画全体でなく current milestone 1個（`currentTask`）だけ**を渡す。全マイルストーンを見せると先のステップへ先走る（実測：歌詞で m1「モチーフを書き出す」を飛ばし m2 の詞を直書き）。

**計画の質（#2）**: マイルストーンは**それ自体が出せる成果物**になるよう切る（`PLAN_SYSTEM` の new_goal 誘導）。「アイデアを出す/準備する」等の meta/下準備ステップは成果物にならず達成判定できない（processor が正しく✓を渋り停滞する）。

**3パターン実測（2026-06-19）**: ①まとめ（出せる成果物）→ **4/4 完遂**・静穏へ自然卒業。②歌詞（meta milestone「モチーフを書き出す」）→ 0/4 retired（processor が正直に未達判定＝#2 が要る証明）。③歌詞（出せる成果物に切り直し）→ #1+#2 で **0/4-retired から 2/4-前進・卒業せず**に改善。**残課題（軟らかい縁）**: 創作のマイルストーン"完了"はファジー（doer が自由詩で書き流し「サビ＝この2行」と切り出されない→判定が渋る）。構造の壊れでなく創作領域＋小モデルの縁。分析・まとめ・調査系は完遂する。
...
plan processor を入れた時点で、計画まわりが性質の違う3層に分かれると整理した（基本構成として合意・Tier 0 のみ実装済み）。

- **Tier 0 ＝ plan processor（実装済み）**: 1つの plan の中で current milestone を1つずつ前へ。線形・**前進のみ**。狭い yes/no 判定×機械適用。
- **Tier 1 ＝ plan オーケストレーター（構想）**: 1つの plan を「**全体を確認して**回す」層。(1) いま turn.ts に散らばる plan ライフサイクル（前判定・入口/sticky/疲労/見限り/達成）を1モジュールに集約。(2) 線形前進に加え**全体レビュー＋手戻り**（全体を統合して見て前とのつながりが悪ければ milestone を reopen して書き直す＝創作の手戻りモデル）。⚠️ 全体レビューは**広い判断**＝信頼性のため避けた狭い判定の逆なので、**毎ターンでなくトリガー時（完了/停滞/節目）限定**・手戻りは reopen+revise に絞る。分析/まとめ系は Tier 0 線形で足り、**Tier 1 が本当に効くのは創作の coherence**＝優先度は「創作がボトルネックになったら」。
- **Tier 2 ＝ plans-to-do 管理（構想・本丸）**: 複数 plan の**ポートフォリオ**層。今は focusPlan が1個でたまたま set された plan しか追えない。未完 plan の backlog インデックス＋「次に何をやるか」の選択＋古い plan の復帰（cooldown を包含）。⚠️ 選択を**中央スケジューラ（ジャッジ復活＝哲学違反）にしない**＝recall/importance で"気がかりな未完 plan"が浮かぶ**観測駆動**にする。**これは「ドライブ／自発性」（ARCH-NEXT「とっさ」/embodied）の土台と地続き**＝idle で未完 plan が浮かぶ→集中に点火、の自発の発電機。ここね（embodied-claude）比で"自発的に動けない"の核心はこの層の不在。

順序の見立て: **Tier 2（自発性の土台・高レバレッジ）を先に設計**、Tier 1 の全体レビューは創作品質がボトルネックになってから。ただし Tier 1 の「コード集約」だけは Tier 2 を載せる土台として先行する価値あり。

Tier 2 を「動かせる」ための前提＝**plan を掴む手（plan actor を本物の効果器に）**。現状 plan actor は `ctx.planId`（focusPlan）だけを編集でき、既存 plan の一覧/別 plan の target/`activate`(開始・再開)/`shelve`(棚上げ)/`retire`(見限り)/自己起点 が無い＝「思い出した plan を掴めない」。設計は [PLAN-FACULTY.md](PLAN-FACULTY.md)（進行係 processor／手 plan actor／目覚まし schedule の3係・new_goal は activate フラグで「今やる/積むだけ」を分け・shelve と retire は別物・cron は暫定）。
...
- **記憶 actor (`memory`)** = recall + forget を統合。activate が1判断で op∈{想起, 忘却}＋intent を出し（強制ギプス＝plan/memo と同型・ジャッジではなく自律アクターが自分の op を選ぶ）、run が `runRecall`/`runForget` に振る（recall は多段ループ廃止＝機械 top-2 提示）。エピソード記憶に対してできるのは **Read(想起)+Delete(忘却) のみ＝受動**。能動的 Create/Update は無い（書き込み＝符号化は内心更新の importance 採点であって直接 write op ではない）。
...
- **完遂前提（半移行で放置しない）**: 分割実装するが Done に到達する＝全 effect（発話/notes/plan/カメラ/MCP）が効果器経由・特別経路ゼロ・`TurnResult.speech` の pull 撤去まで。計画とチェックリストは [EFFECTORS-PLAN.md](EFFECTORS-PLAN.md)。
