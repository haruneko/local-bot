...
**内心の焼き直し防止**: `updateInnerState` に渡す `prevInnerState` は最初の1文（句点区切り）に切り詰める。フル文を渡すと変化が小さいターンで前の内心の言い換えになりがちなため。内心更新プロンプトは「このターンの出来事を主な根拠に書く、前の内心は参照程度」とする。
...
| `heartbeat` | 直近ユーザー発話 → 前回発話（`lastAssistantContent`、独り言含む） → `innerState`（ムードベース） → **null（recall スキップ）** |

- heartbeat かつ何も種がない（= 会話も発話も内心も空）場合は `null` を返し、recall・semantic・memoIndex の検索を全スキップする
- 前回発話を種にする理由: 「次は〇〇を調べよう」という独り言が次のハートビートの検索クエリになり、タスク継続性が生まれる
- `innerState` をムードとして下位に置く理由: 感情クエリは「今の気分に合った記憶」を引くが、「何を考えているか」の継続性には弱い。フォールバックとして残す
...
`roles` のキーは `language` / `introspection` / `innerState` の3つのみ。v0.6 時代の `memory` / `research` キーは廃止。
