...
### steps processor（前判定・計画実行の背骨）（2026-06-19）

**実機計測で判明した故障**: 集中で heartbeat を連打しても**計画が前進しない**。プローブ（マイルストーン付き計画を集中で8連打）で、生成(synthesize)は毎ターン works に書くのに **done=0/4 のまま固定→空回り→見限り(retired)**。原因の連鎖：①マイルストーンを進める唯一の経路 `steps` actor が **multi-label で「synthesize が回ってるから steps 不要」と判定され8回中1回しか発火しない**。②その1回も `complete` でなく `log` を選ぶ。③ current が動かず計画チャンネルが永遠に「m1」を指す→ synthesize が同じ所を再記述しループ。④進捗センサ(`stepsProgress`=done+log)は works を見ないので、書いてるのに「進捗ゼロ」と盲目見限り。＝**前進を「steps actor が発火するか＋正しい op を選ぶか」という広い LLM 判断に賭けていたのが誤り**（codex 教訓：進捗追跡と step 遷移はモデルに委ねずシステムが握る）。

**解＝steps processor を前判定の機械フェーズに**（`src/roles/steps-processor.ts`・SPEC §4.2 phase 1.5）。集中の毎ターン頭で、成果物(works)と計画を突き合わせ、current から前へ「この milestone は成果物の中で達成されたか」を**狭い yes/no で1つずつ**判定（分割統治＝小モデルで固い）。満たされた milestone を**機械が✓・current 前進**、全✓なら締める。LLM の仕事は二値判定だけ・構造更新（✓・current・達成ログ）はコード。これで「steps actor 発火の博打」も「盲目見限り」も消える。steps actor は計画の**作成/編集**役に縮小（前進は processor 担当）。配置は**前判定**を選択：完了が「決めてから締める」きれいな流れになり、書く人が常に正しい milestone を書く保証が最強（前ターンまでの全 works から毎ターン頭で実態に合わせる＝ズレ窓ゼロ）。「判定の配置(前/後)」と「喋りの任意化」は別レバー＝混同しない。
...
**計画の質（#2）**: マイルストーンは**それ自体が出せる成果物**になるよう切る（`STEPS_SYSTEM` の new_goal 誘導）。「アイデアを出す/準備する」等の meta/下準備ステップは成果物にならず達成判定できない（processor が正しく✓を渋り停滞する）。
...
- **Tier 1 ＝ steps オーケストレーター（構想）**: 1つの steps を「**全体を確認して**回す」層。(1) いま turn.ts に散らばる steps ライフサイクル（前判定・入口/sticky/疲労/見限り/達成）を1モジュールに集約。(2) 線形前進に加え**全体レビュー＋手戻り**（全体を統合して見て前とのつながりが悪ければ milestone を reopen して書き直す＝創作の手戻りモデル）。⚠️ 全体レビューは**広い判断**＝信頼性のため避けた狭い判定の逆なので、**毎ターンでなくトリガー時（完了/停滞/節目）限定**・手戻りは reopen+revise に絞る。分析/まとめ系は Tier 0 線形で足り、**Tier 1 が本当に効くのは創作の coherence**＝優先度は「創作がボトルネックになったら」。
