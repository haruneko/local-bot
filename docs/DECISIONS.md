...
**着手点**: 事実記録テーブル（turnId キー）の新設・`persistReflection`（`src/orchestrator/turn.ts`）で事実記録を書く・`runDream`（`src/roles/dream.ts`）が id で事実を引いて蒸留・`INTROSPECTION_SYSTEM`（`src/prompts/roles.ts`）に保険1行。
