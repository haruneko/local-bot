...
- **memory-agent / research-agent**: Ollama `format`（JSON Schema）で `{ activate: false }` または `{ activate: true, tool: "...", intent: "..." }` を出力。`activate: true` のとき即実行。パース失敗・リトライ失敗 → `{ activate: false }` にフォールバック。
...
- **認知の構造（2フェーズ）**: 入力（プリプロセス）→ 自律エージェント（memory / research / language → 内省/Memory）。プリプロセスが起点で、全エージェントが同一 `TurnContext` を参照する
...
- 1ターンにつき **1つの `TurnContext`** を作成し、preprocess → memory-agent → research-agent → language-agent → 内省の各段階で **同じオブジェクトを更新**する
- ターン終了時に破棄する（永続化は内省→LanceDB のみ）
- 全エージェント・内省は **同じ事実データ**（相手発話・直前会話・想起）を参照する。system プロンプトと出力形式だけがロールごとに異なる
- ロールごとに別フォーマットで入力を組み立てない（`TurnBrief` / `VolatileContext` 二重経路は廃止）
- 全エージェントは `memorySnapshot(ctx)` から同じ事実フィールドを参照する
...
- `ctx.actions: ActionOutcome[]`（memory-agent → research-agent の順で追加）
