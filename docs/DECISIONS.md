...
**未着手（本丸の残り）**: ②符号化側のロンダリング。内省は `ctx.speech`（作話を含みうる）と ActionFacts（裏打ちあり）の両方を見ているのに、**発話より行動結果を信頼せよという重み付けが無い**。すり抜けた作話が記憶に焼かれる経路は残っている（INTROSPECTION_SYSTEM に「ActionFacts に裏打ちのある出来事を事実として書く」を入れる案・未着手）。
...
- **記憶 actor (`memory`)** = recall + forget を統合。activate が1判断で op∈{想起, 忘却}＋intent を出し（強制ギプス＝plan/memo と同型・ジャッジではなく自律アクターが自分の op を選ぶ）、run が `runRecallLoop`/`runForget` に振る。エピソード記憶に対してできるのは **Read(想起)+Delete(忘却) のみ＝受動**。能動的 Create/Update は無い（書き込み＝符号化は内省の importance 採点であって直接 write op ではない）。
