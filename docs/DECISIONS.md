...
~~`recall` 行動のヒット要約も LLM（`summarizeRecallActionHits`）。~~ **2026-06-18 廃止**: recall の多段ループと要約 LLM をやめ、ベクトル検索の上位を**機械的に top-2 提示**する（`runRecall`）。常時走る背景 recall と二重で費用倒れだったため（約3倍速・想起内容も決定的に。research/recall-mechanical-2026-06-18.md）。下記「取り込み（主犯）」の機構もこれで消えた。
...
＝**実行・言語化・内省の分離（分離脳）は load-bearing**。曲げると作話対策が無に帰し、汚れた内省が episode body＝記憶に焼かれ recall が経時劣化する。3秒の節約で記憶の正直さを売る取引＝不採用。証拠: `research/merge-lang-intro-refutation-2026-06-18.md`。speed が要るなら**非同期 reflect**（発話を先に返し内省/affect は返答後）＝芯を保ったまま体感を縮める（ARCH-NEXT「とっさの返事」）。
...
- 反例（採らない）: 発話だけ orchestrator presenter コールバックで特別扱い＝非対称で不採用（→効果器に揃える）。reflection を1発に畳む案も不採用（[[project_activator_cheap_gate_failed]]／research/merge-lang-intro-refutation）。
