...
- 設計詳細: [MEMO-TREE.md](MEMO-TREE.md)（連想ディセント読み・MOC ツリー・op 純関数 applier）
...
- **集中への事後入室**: 言語野任せだと「会話中だから対話」を選びがち。**plan op が成功したターンは orchestrator が NEXT_STATE を集中に上書き**（目標に取り組んだ事実への事後反応。抜けるのは次ターン以降の言語野判断）。← 言語化＝結果であって原因でない、というコンセプト通り。
