...
## 想起提示の LLM 要約廃止＝本文そのまま（2026-07-14）

**決定**: エピソード想起の summarize 帯（距離 fullMax〜summarizeMax）を LLM で要約して提示するのをやめ、omit ゲートを通った本文を**そのまま**提示する（`src/recall/present.ts`・純関数・旧 `llm-present.ts` は削除）。

**根拠（発話レイテンシ診断・Strix Halo 実測）**:
- ホストは Strix Halo（LPDDR 128GB ユニファイド・帯域 ~256GB/s）で qwen3.6:35b-a3b は **decode 57 tok/s / prefill 1,000+ tok/s（キャッシュヒット時ほぼ無料）**＝完全に decode 律速。ターン所要 ≒ 経路上の生成トークン総量。
- summarize 提示は 35B が要約を 150〜200 tok 生成＝**毎ターン約3秒**。それで節約できる言語野プロンプトは数百 tok＝prefill 換算 0.3 秒未満。**0.3 秒のために 3 秒払う逆ザヤ**。
- エピソード本文はもともと短い（内省1本 100〜200字）ので full で渡しても誤差。LLM 要約は言語野が読む前に記憶を歪める劣化変換（メモ本文を LLM 要約しない、と同じ理屈）＝作話の注入口でもあった。
- 機械 truncation（先頭80字）も不採用: 文の途中でちぎれた断片は旧 vague 帯廃止（中身ゼロのノイズ）と同じ失敗形状。
- 並列化（想起提示を activator と同時に走らせる）も不採用: MoE は 2 ストリーム同時 decode で別々のエキスパートを読む＝重み読みがほぼ2倍＝実測で直列とほぼ同じ（6.8s→6.5s・約5%）。GPU スループット律速では重ね合わせで稼げない。

**遠さの表現はどうなるか**: 「ふんわり思い出す」のグラデーションは、relevance（距離×recencyDecay×importance×抑制×話者ブースト）の並び順と `occurredAt` の `[N分前/N日前]` 前置きが担う。遠い記憶に引っ張られる症状が出たら、戻すツマミは LLM 復活ではなく **summarizeMax を下げて omit を増やす**（想起を狭める）。

**残置**: `recallDistance.fullMax` は提示に効かなくなった（omit ゲート＝summarizeMax・relevance 正規化＝vagueMax のみ有効）。設定互換のため残す。トークン予算超過時の `fitTurnContext` フォールバック要約（SPEC §9）は最終手段として温存＝「（要約）」タグはそこ専用。

## 研究→記録の実行順序＝memo の転記元グラウンディング（2026-07-14）

**発端**: 10ターンラリー監査で「調べてメモして」型のターンに memo と webSearch が multi-label で同時発火し、並列実行のため memo は検索結果を見られず、**自前の重み（一般知識）から捏造した概要を notes に書いた**（Stack-chan を Vocaloid/Unity/VRChat のプロジェクトと記載＝完全な作話がディスクに永続化）。

**決定**（turn.ts `runActorPool`）:
- 実行順序を **研究系（webSearch/urlBrowse 等）→ memo → steps** に直列化。memo 実行時には同ターンの調べ物の実結果が `ctx.actions` に積まれている。
- `runMemo` の op プロンプトに「このターンであなたが実際にやったこと（転記に使ってよい実結果）」として succeeded な `ctx.actions` の facts を渡す（`formatActionFactContent`・steps 受け入れ判定と同じ整形）。
- `MEMO_OP_SYSTEM` に転記境界を明記: 書ける中身は与えられた文脈（会話・このターンの行動実結果・意図・対象メモ）にある事実だけ。**文脈に無いことを一般知識で補って書かない**。
- memo の criteria にも境界: 転記できる実物があるときだけ／調べる依頼だけ（記録まで頼まれていない）では起動しない／調べて控える依頼なら webSearch と同時起動してよい（実行は調べたあと）。

**根拠**:
- memo の役割は転記（生成は synthesize のレーン）。転記元が構造的に見えないのに書かせるのは作話の招き（§幻覚への構え の「招きを断つ」と同型）。steps が「他 actor を先に実行してから最後に走る＝結果でグラウンディング」の前例。
- 直列化のレイテンシ代はほぼゼロ: decode 律速（§想起提示の LLM 要約廃止）で LLM 同士の並列は実測直列とほぼ同速。失うのは研究のネットワーク待ちとの重ね合わせ（1〜2秒・該当ターンのみ）。
- 併せて steps actor の stepsId 幻覚（存在しない「m1」を2/10ターンで参照→invalid_args）も修正: `STEPS_SYSTEM` が段取り一覧の id 例を「(m1) のような」と誤って教えていた（実レンダリングはタイトルがそのまま id・m1/m2 はマイルストーン専用）。例示を実態に合わせた。

## 口＝音声効果器と文単位ストリーミング（2026-07-14）

**決定**: 口（OutputChannel）に音声を実装。3層分離＝①`LlmClient.chatStream`（差分ストリーム・リミッタのスロットは反復終了まで保持・ストリーム途中の再試行は二重出力になるので持たない）②文単位化（`speech-stream.ts`＝grammar 強制 JSON の `speech` 値を incremental 抽出→文分割。**正本は完了後の全文パース**＝ストリーミングは提示専用）③音声シンク（`src/voice/`＝VOICEVOX 合成→直列再生キュー→テキスト先行表示・失敗はその文スキップ）。層の境界＝出口の差し替え点（スタックチャン到着時は③の再生先を HTTP に変えるだけ・ARCH-NEXT §スタックチャン統合スケッチ）。

**根拠**: decode 律速（§想起提示の LLM 要約廃止・57tok/s）では全文完了待ち（≈2.5-3秒）が体感の支配項で、文単位ストリーミングなら最初の一文≈1-2秒。実測: `--voice` 実ターンで2文が別々に流れ、1文目は言語野開始から約2秒で発声。

**運用**: VOICEVOX ENGINE は WSL2 の Docker（`voicevox/voicevox_engine:cpu-latest`・127.0.0.1:50021 バインド・localhost 完結）。**常駐させない**（restart=no・メイン機は DAW/ゲームと共用のため載せ過ぎない）＝必要時に `/voice` スキル（`.claude/commands/voice.md`）で start/stop する。エバ側は ENGINE 不在でも degrade でテキスト続行するので stop したままでも壊れない。Windows 側 GUI は声の試聴用途のみ（`/speakers` API でも代替可）。

**反例（採らない）**: ストリーミング済みの文を正本にすること（パース失敗・途中エラー時に作業記憶と発話がズレる）／artifacts の読み上げ（長文 dump が音声を占拠する・TurnResult.artifacts の設計どおり宛先判断で出さない）。

## 耳＝STT は gemma4:e2b（Ollama 直・whisper 不採用）（2026-07-14）

**決定**: マイク音声の文字起こしは **`gemma4:e2b`（Ollama の OpenAI 互換 `/v1/chat/completions`・`input_audio`・16kHz mono s16le wav 必須）**。`--talk` モード（REPL で空行 Enter＝録音トグル→文字起こし→user_message・生 wav は `trigger.audio` に併載＝audio_feed へ）。

**根拠（実測 2026-07-14）**: TTS→STT 往復で文字起こし完全一致・warm 3秒。whisper.cpp（large-v3-turbo・Docker）も動いたが未チューニングで15秒＋**コンテナが1個増える**。gemma4:e2b は既存 Ollama に乗り keep_alive で自動アンロード＝「メイン機に載せ過ぎない」に適合。**e4b の audio 経路は壊れている**（16kHz でガベージ・24kHz で不達）ので使わない。whisper モデルは `~/models/whisper/` に残置（実マイクで e2b の精度が足りない時の代替）。文字起こしを正本にするのは「文字起こししない」原則（ARCH-NEXT・非言語知覚の話）と矛盾しない＝相手の発話は言語内容そのもの。将来 omni（Qwen3-Omni 系）が Ollama に乗れば耳ごと差し替え可。
