...
**計画の質＋重複防止**: マイルストーンは**出せる成果物**単位で切る（`STEPS_SYSTEM` の new_goal 誘導・meta/下準備ステップは達成判定できない）。new_goal は既存の全段取り（完了済み含む）と字句照合し、似たものがあれば作らない（重複生成の防止）。

**実機**: まとめ（作る系）4/4 完遂。「判定の配置(前/後)」と「喋りの任意化」は別レバー。**残課題**: 創作の"完了"はファジー（自由詩で「サビ＝この2行」と切り出されない）／研究系の実機確認（Tavily 込み）は未。idle surface は重複防止後に ON（間引き: `IDLE_SURFACE_EVERY`=3 回に1回）へ復帰。
