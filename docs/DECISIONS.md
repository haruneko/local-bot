...
**実機**: まとめ（作る系）4/4 完遂。「判定の配置(前/後)」と「喋りの任意化」は別レバー。**残課題**: 創作の"完了"はファジー（自由詩で「サビ＝この2行」と切り出されない）／研究系の実機確認（Tavily 込み）は未／**自律 idle が"進められない段取り"を拾って空回り**（→「idle は前に進められるものだけ拾う」で断つ・未）。idle surface は重複防止後に ON（間引き: `IDLE_SURFACE_EVERY`=3 回に1回）へ復帰。
