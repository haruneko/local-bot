...
- **計画"実行"の前進（2026-06-19・実装済み）**: 実機計測で「集中で連打しても milestone が進まない」「研究を作話する」を発見し、**受け入れ判定（実行後・実結果でグラウンディング）＋ dispatcher（手選び）＋完了畳み＋重複防止**で解決（DECISIONS §集中モード）。idle surface は OFF 据え置き。
