...
**進捗（2026-06-15）**: 受動の視覚 ✅、Slack 画像受信 ✅（実機確認済・花束の写真でエバが反応）。＝「目」が通った。残り = 能動の見にいく・画像の記憶 v2/v3・音声・ARCH-NEXT 本体。
...
- **v1（受動の視覚に内包・実装済の範囲）**: 内省が「会話で起きたこと＝エバの反応」をエピソードに書く。画像のキャプションではなく彼女の言葉。画像バイトはそのターンで消す（揮発・とっさの一言と同じ扱い）。
- **v2**: 画像を `data/images/<turnId>.jpg` に保存し、エピソードにパスだけ紐づける（正本はピクセル、言葉はただの取っ手）。想起はテキストで起こし、その記憶が**焦点になった時だけ**実画像を再添付して見直す（毎回は重いので焦点時のみ）。古い画像の間引きは後で決めるツマミ。
- **v3**: 画像で画像を思い出す（視覚的類似）。画像用の埋め込みモデル（CLIP 系）が要る。サーバに今は無い（`nomic-embed-text`＝テキストのみ）ので pull する話。別レイヤー・一番後。
