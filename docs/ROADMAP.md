...
- **符号化ロンダリングの本丸**（🟡 設計済・実装これから）: 作話を招く経路を断つ A/B/C は実装済み。importance は内心更新側へ移した。残る本丸＝「内省が事後に語った内容が、検証なく確信的な記憶として固まる」流れ。**対策設計**（2026-06-15）＝エピソードを「コードが機械生成する事実の背骨（ActionFacts 裏打ち）＋内省の上塗り（感じたことの層）」に割り、LLM に事実欄を書かせない。詳細は [DECISIONS.md](DECISIONS.md) §②符号化側のロンダリング対策。
