...
| 2 | MOC ツリー（`_index.md` 機械再生成）＋ 連想ディセント読み | ✅ 済 | `memo/tree.ts` `memo/descent.ts` |
| 3 | recall 加速：**フォールバック**（迷子救済テレポート） | ✅ 済 | `descent.ts` `recallFallbackTarget` |
| 3' | recall 加速：入口（最有力サブツリーへ）/ 剪定（各層で子をスコア順上位だけ提示） | 後回し | 木が深く/フォルダが太くなったら合成。現状の浅い木では挙動不変の最適化のため未実装 |
| 4 | サイズ自動分割（予算超過を見出し境界でフォルダ化・verbatim） | ✅ 済 | `memo/tree.ts` `splitIfOversized` |
| — | 見出し無しベタ書き巨大ファイルの byte ページング | 将来・エッジ | `splitIfOversized` は分割不能として null を返す |
...
### read→op の LLM 呼び出し回数（実測の目安）

- 空ツリー / 新規 create: descent はスキップ（0）＋ op（1）＝ **1回**
- ルート直下の葉: descent（1）＋ op（1）＝ **2回**
- フォルダを N 段降りる: descent（N+1）＋ op（1）。木が浅いうちは 1〜2 段。
- 迷子（descent 行き止まり）: descent（行き止まりまで）＋ フォールバック想起（LLM 不要）＋ op（1）
