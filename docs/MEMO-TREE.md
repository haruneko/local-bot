...
**hybrid（字句×意味・2026-06-17）**: 「名前そのまま」クエリ（ファイル名を直に言う）は意味ベクトルが弱い。そこでファイル名の**文字バイグラム Dice**（`src/recall/lexical.ts`）で字句一致チャンネルを作り、ベクトル想起と **RRF 融合**して候補一覧を作る。ただし**ゲート**（Dice > `LEXICAL_MIN_DICE`=0.4 のときだけ融合）で、話題クエリへのノイズ注入を防ぐ。`eval:retrieval --hybrid` で実測（素朴な等重み融合は全体を壊す＝ゲート必須）。これは候補の並べ方の改善で、最終判断は LLM 認識のまま。
