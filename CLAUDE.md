...
- **actor pool** (`src/actors/`): `memory` `memo` `webSearch` `urlBrowse` `webcam` `plan` `synthesize` が独立して並列に起動判断・実行。**記憶系は「記憶／記録」の2 facultyに分割統治**（B'・[docs/DECISIONS.md](docs/DECISIONS.md) §記憶 faculty）: `memory`＝**受動の記憶**（エピソード記憶。**能動想起＝recall 専用**＝activate が「自分から思い出しにいくか」を判断し run が `runRecall`＝ベクトル検索の上位を**機械的に top-2 提示**＝LLM 要約しない・作話面で安全。エピソードは Read(想起) のみ＝能動 Create/Update は無い・符号化は内心更新の importance 採点。**忘却は op でなく減衰**＝`recencyDecay`×importance で自然に出なくなる＝「意識して忘れる」は人間にないため forget op は廃止(2026-06-18)。本気の削除はプライバシー用 out-of-band（`runForget` は温存）。`remember` 廃止の理屈そのもの。`EpisodeSource "remember"` と present 表示は履歴用に温存）／ `memo`＝**能動の記録**（notes ファイルの full CRUD）。各 actor が `activate()` で自己判断し、起動した actor のみ実行。mini-context（直近3ターン）で判断。`plan` は集中モードの**構造化plan**（`data/plans/<id>.json`）を op で更新する（構造はコードが保証・LLM は op を1つ出すだけ。markdown は `data/notes/goals/` への派生ビュー）。`memo` は読み書き統合 actor で、locate（主=recall認識・フォールバック=連想ディセント）→対象を全文ロード（read-before-edit・**行番号付きで提示**）→ op を1つ（view/create/append/replace/section_replace/**replace_line**/**delete_line**）を純関数 applier で適用する（[docs/MEMO-TREE.md](docs/MEMO-TREE.md)）。`synthesize` は想起＋外部＋感性（内心/関心事）を統合して成果物（歌詞・読書メモ・まとめ・文章）を**生成**し `data/notes/works/<planId|slug>.md` へ append で外化する＝「行動としての思考」。memo（強制ギプス・転記）と違い**生成が役割**の唯一のレーン。「書いて/作って/まとめて」や集中作業の成果物前進で起動。
